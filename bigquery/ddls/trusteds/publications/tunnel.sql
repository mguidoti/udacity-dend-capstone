CREATE TABLE IF NOT EXISTS `poc-plazi-trusted.publications.tunnel`
(
  ...
  source                    STRING      OPTIONS(description="Source of the row. The publication tunnel will run in the platform and locally on people's machines, which in turn will have their own dedicated raw (apis.notion_publications_tunnel)."),
...
OPTIONS (
    description='Table that records any status update from the applications running the publications tunnel in and out the platform.'
)